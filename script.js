...
var app = new Vue({
    ...
    methods: {
        createTodo: function() {
            fetch(`${baseAddress}/api/missions`, { method: "POST",
                body: JSON.stringify({ missionDescription: this.newTask} )})
        ...
        deleteTodo: function(todo) {
            ...
            fetch(`${baseAddress}/api/mission/${todo.id}`, { method: "DELETE"})
                .then(function() {
        ...
        updateTodo: function(todo) {
            ...
            fetch(`${baseAddress}/api/mission/${todo.id}`, 
                { method: "PUT", body: body})
                .catch(reason => this.error = `Failed to update item: ${reason}`);
        },     
    },
    mounted: function () {
        fetch(`${baseAddress}/api/missions`)
            .then(response => response.json())
